...
def lint(c):  # type: ignore
    ...
    c.run("poetry run pylint --rcfile pyproject.toml -j 0 aws_in_docker > pylint-report.txt")


@task(aliases=["tests"])
def test(c):  # type: ignore
    """Run tests quickly with the default Python."""
    c.run("PYTHONPATH=./aws_in_docker poetry run pytest -v")
...
def coverage(c):  # type: ignore
    ...
    c.run(
        """
        poetry run coverage run --branch --source=aws_in_docker \
            -m pytest --junitxml=ut-report.xml tests/
    """.strip()
    )
    c.run("poetry run pylint --rcfile pyproject.toml aws_in_docker -r n")
...
def enforce_coverage_render_tests(c):  # type: ignore
    ...
    c.run(
        """
        poetry run pytest --cov=aws_in_docker --cov-fail-under=80 --cov-branch \
    ...
    try:
        c.run(
            ...
            poetry run pylint --rcfile pyproject.toml \
                -j 0 aws_in_docker > {pylint_output_file_name}
        """.strip()
        )
